...
function Input(){
  ...
  if(cache.length>3){cache.shift();console.log('cache is full, removing oldest item')}
  const debounce = (func)=>{
    let timer;
    return function(...args){
        const context=this;
        if(timer)clearTimeout(timer)
        timer=setTimeout(()=>{timer=null;func.apply(context,args)},1000)}}
        const memeTime=useCallback(debounce(getData))

  async function getData(event){
    
    const {value} = event.target;
    if(cache.length>0&&cache.filter(obj=>obj.c_loc.toLowerCase()==event.target.value.toLowerCase()).length>0){
      var c_obj = cache.map(obj=>{return obj.c_loc.toLowerCase()}).indexOf(event.target.value.toLowerCase())
      ...
      console.log('showing cached '+value+', no API call needed')
    ...
    else if(value==''){setCity('');setTemp('');setWeatherCond('')
    }
    else{
        ...
        .then(data => {
            setCity(data.location.name);setTemp(data.current.temp_c + '°C');setWeatherCond(data.current.condition.text);
            cacheThis(cache=>[...cache,{
